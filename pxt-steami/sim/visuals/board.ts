...
.sim-board-button:hover {
    ...
    export function getBoardDimensions(
        vis: BoardImageDefinition,
    ): ComputedBoardDimensions {
    ...
    export class MetroBoardSvg extends GenericBoardSvg {
        ...
        constructor(public props: MetroBoardProps) {
            ...
            for (const l of props.visualDef.leds || []) {
                if (l.color == 'neopixel') {
                    const onBoardNeopixel = new BoardNeopixel(
                        l.label,
                        l.x,
                        l.y,
                        l.w || 0,
        ...
        public updateState() {
            ...
            if (this.board.neopixelPin) {
                const state = this.board.neopixelState(
                    this.board.neopixelPin.id,
                );
                if (state.buffer) {
    ...
    class BoardResetButton {
        ...
        constructor(p: BoxDefinition) {
            ...
            pointerEvents.down.forEach(evid =>
                this.element.addEventListener(evid, ev => {
    ...
    class BoardLed {
        ...
        updateState() {
            const opacity =
                this.pin.mode & PinFlags.Digital
                    ? this.pin.value > 0
                        ? 1
                        : 0
                    : 0.1 +
                      (Math.max(0, Math.min(1023, this.pin.value)) / 1023) *
                          0.8;
    ...
    class BoardNeopixel {
        ...
        setColor(rgb: [number, number, number]) {
            ...
            l = (l * 90) / 100 + 10;
            this.element.style.stroke = `hsl(${h}, ${s}%, ${Math.min(
                l * 3,
                75,
            )}%)`;
